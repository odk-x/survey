...
public class MainMenuActivity extends SherlockFragmentActivity implements
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		outState.putBoolean(PROCESS_APK_EXPANSION_FILES,
				mProcessAPKExpansionFiles);
	...
	protected void onResume() {
		...
		if (mgr.getBackStackEntryCount() == 0) {
			...
			if ((nestedScreen != ScreenList.COPY_EXPANSION_FILES)
				...
				ArrayList<Map<String, Object>> files = Survey.getInstance()
						.expansionFiles();
				if (files != null
						|| CopyExpansionFilesTask
								.debugAPKExpansionFileProcessing()) {
	...
	public void setCurrentForm(FormIdStruct currentForm) {
		Survey.getInstance()
				...
				.i(t,
						"setCurrentForm: "
								+ ((currentForm == null) ? "null"
										: currentForm.formPath));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			pageWaitingForData = savedInstanceState
					.containsKey(PAGE_WAITING_FOR_DATA) ? savedInstanceState
					.getString(PAGE_WAITING_FOR_DATA) : null;
			pathWaitingForData = savedInstanceState
					.containsKey(PATH_WAITING_FOR_DATA) ? savedInstanceState
					.getString(PATH_WAITING_FOR_DATA) : null;
			actionWaitingForData = savedInstanceState
					.containsKey(ACTION_WAITING_FOR_DATA) ? savedInstanceState
					.getString(ACTION_WAITING_FOR_DATA) : null;

			currentScreen = ScreenList.valueOf(savedInstanceState
					.containsKey(CURRENT_SCREEN) ? savedInstanceState
					.getString(CURRENT_SCREEN) : ScreenList.MAIN_SCREEN.name());
			nestedScreen = ScreenList.valueOf(savedInstanceState
					.containsKey(NESTED_SCREEN) ? savedInstanceState
					.getString(NESTED_SCREEN) : ScreenList.FORM_CHOOSER.name());
			if (savedInstanceState.containsKey(PROCESS_APK_EXPANSION_FILES)) {
				mProcessAPKExpansionFiles = savedInstanceState
...

		{
			...
			view.loadPage(getCurrentForm(), getInstanceId(), getPageRef(),
					getAuxillaryHash());
		}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (currentScreen == ScreenList.MAIN_SCREEN) {
			boolean get = mAdminPreferences.getBoolean(
			...
			boolean manage = mAdminPreferences.getBoolean(
					AdminPreferencesActivity.KEY_MANAGE_FORMS, true);
			if (manage) {
			...
			boolean send = mAdminPreferences.getBoolean(
		...
		boolean settings = mAdminPreferences.getBoolean(
	...
	public void chooseForm(Uri formUri) {
		 ...
		 * (Intent.ACTION_PICK.equals(action)) { // caller is waiting on a
		 * picked form getActivity().setResult(Activity.RESULT_OK, new
	...
	public void onBackPressed() {
		if (currentScreen == ScreenList.WEBKIT
				&& nestedScreen == ScreenList.WEBKIT && getInstanceId() != null) {
			JQueryODKView view = (JQueryODKView) findViewById(R.id.webkit_view);
			view.goBack();
		} else {
			FragmentManager mgr = getSupportFragmentManager();
	...
	private void createErrorDialog(String errorMsg, final boolean shouldExit) {
		...
		mAlertDialog.setButton(DialogInterface.BUTTON_POSITIVE,
				getString(R.string.ok), errorListener);
		mAlertDialog.show();
	}
	...
	public void hideWebkitView() {
		...
		this.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				Survey.getInstance().getLogger()
						.log(WebLogger.INFO, t, "hideWebkitView");
	...
	public void swapToFragmentView(ScreenList newNestedView) {
		...
		if (newNestedView == ScreenList.MAIN_SCREEN) {
			throw new IllegalStateException(
					"unexpected reference to generic main screen");
			...
			((CopyExpansionFilesFragment) f).setFragmentToShowNext(nestedScreen
					.name());
			newCurrentView = ScreenList.MAIN_SCREEN;
	...
	public void onActivityResult(int requestCode, int resultCode, Intent intent) {
		...
		if (requestCode == HANDLER_ACTIVITY_CODE) {
			try {
				...
				view.loadJavascriptUrl("javascript:landing.opendatakitCallback('"
						+ pageWaitingForData
						+ "','"
						+ pathWaitingForData
						+ "','"
