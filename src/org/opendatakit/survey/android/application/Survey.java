...
public class Survey extends Application implements LicenseCheckerCallback {
	...
	public static int getQuestionFontsize() {
		...
		String question_font = settings.getString(
				PreferencesActivity.KEY_FONT_SIZE, Survey.DEFAULT_FONTSIZE);
	...
	public static boolean createODKDirs() throws RuntimeException {
		...
		for (String dirName : dirs) {
			...
			if (!dir.exists()) {
				if (!dir.mkdirs()) {
					RuntimeException e = new RuntimeException(
							"ODK reports :: Cannot create directory: "
									+ dirName);
					throw e;
				}
			} else {
				if (!dir.isDirectory()) {
					RuntimeException e = new RuntimeException("ODK reports :: "
	...
	public void onCreate() {
		...
		SharedPreferences settings = PreferenceManager
				.getDefaultSharedPreferences(Survey.getInstance());
		String saltString = settings.getString(PreferencesActivity.KEY_SALT,
				null);
		do {
			if (saltString == null) {
				...
				settings.edit()
						.putString(PreferencesActivity.KEY_SALT, saltString)
						.commit();
			} else {
				try {
					...
					Log.e(t,
							"Unable to decode saved salt string -- regenerating");
					saltString = null;
					e.printStackTrace();
				}
			}
		...
		mAPKExpansionPolicy = new APKExpansionPolicy(this, new AESObfuscator(
				mSalt, getPackageName(), deviceId));
		mLicenseChecker = new LicenseChecker(this, mAPKExpansionPolicy,
	...
	public void allow(int reason) {
		...
		if (mAPKExpansionPolicy.isUpdatedFromServer()) {
			...
			// preferences.
			File f = new File(new File(new File(
			...
			try {
				...
				settings.edit()
						.putString(PreferencesActivity.KEY_APK_EXPANSIONS,
								expansionDefs).commit();
				Log.i(t,
						"persisted the expansion file list ("
								+ expansions.size() + " expansion files)");
	...
	public ArrayList<Map<String, Object>> expansionFiles() {
		File f = new File(new File(new File(
				Environment.getExternalStorageDirectory(), "Android"), "obb"),
				getPackageName());
		f.mkdirs();

		SharedPreferences settings = PreferenceManager
				.getDefaultSharedPreferences(Survey.getInstance());
		String expansionDefs = settings.getString(
		...
		if (expansionDefs != null) {
			try {
				...
				e.printStackTrace();
				Log.e(t,
						"unable to retrieve expected APK Expansion information");
			} catch (JsonMappingException e) {
				e.printStackTrace();
				Log.e(t,
						"unable to retrieve expected APK Expansion information");
	...
	public void applicationError(int errorCode) {
		Log.e(t,
				"applicationError: license check ERROR: "
